function [outCoefs,lamb,alllamb] = ridge_on_basis(inDat, inBasis,lamb,otherargs)
...
if isempty(lamb)
    ...
    [~,outCoefs] = arrayfun(@(i_) lasso(inBasis,inDat(i_,:)','CV',10,'Lambda',ll,'Alpha',0,otherargs{:}) , 1:nObvs, ...
        'UniformOutput' , false) ;
...
outCoefs = cell2mat(arrayfun(@(i_) lasso(inBasis,inDat(i_,:)','Lambda',lamb,'Alpha',0,otherargs{:}) , 1:nObvs, ...
    'UniformOutput' , false)) ;
